...
class FFMMNet(nn.Module):
    def __init__(
            self, dropout=0.3, resolution=256, input_channels=4, output_channel=1, num_layers=6,
            start_reduced_dimension=2, end_reduced_dimension=64
    ):
        ...
        self.start_se = SqueezeExcitation(input_channels, reduced_dim=start_reduced_dimension)

        self.max_pool = nn.AdaptiveMaxPool2d((round(resolution // 2), round(resolution // 2)))
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

        self.end_se = SqueezeExcitation(176, reduced_dim=end_reduced_dimension)
...
class FFMMNet2(nn.Module):
    def __init__(
            self, dropout=0.3, resolution=256, input_channels=4, output_channel=1, num_layers=8,
            start_reduced_dimension=3, middle_reduced_dimension=32, end_reduced_dimension=64
    ):
        ...
        self.max_pool = nn.AdaptiveMaxPool2d((round(resolution // 2), round(resolution // 2)))
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.middle_se1 = SqueezeExcitation(80, reduced_dim=middle_reduced_dimension)

        self.end_se = SqueezeExcitation(224, reduced_dim=end_reduced_dimension)
        self.drop_out = nn.Dropout2d(p=dropout)
        self.final_conv = nn.Sequential(
...
class FFMMNet3(nn.Module):
    def __init__(
            self, dropout=0.3, resolution=256, input_channels=4, output_channel=1, num_layers=5
    ):
