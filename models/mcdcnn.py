...
class FFMMNet(nn.Module):
    def __init__(self, dropout = 0.3, resolution = 256, input_channels = 4, output_channel = 1):
        ...
        self.end_se = SqueezeExcitation(192, reduced_dim=155)
...
class FFMMNet2(nn.Module):
    def __init__(self, dropout = 0.3, resolution = 256, input_channels = 5, output_channel = 1):
        ...
        self.drop_out = nn.Dropout2d(p=dropout)
        self.final_conv = nn.Sequential(
            nn.Conv2d(2, output_channel, kernel_size=1, bias=False),
            nn.BatchNorm2d(num_features=output_channel)
        )
    ...
    def forward(self, masks, mean_std):
        ...
        # Ending
        x = self.drop_out(middle_branch)
        x = self.final_conv(x)
        return torch.sigmoid(x)

class FFMMNet_Old(nn.Module):
